...
def update(username):
    try:
        ...
        if user.role:
            ...
            pass
        form.photo.data = "1.png"
        handles = user.socials # Retrieve existing dictionary or create a new one

        if form.validate_on_submit():
                # Check if the form has a photo field
            if 'photo' in request.files:
                photo_present = True
            else:
                photo_present = False
            
                # Return a response including keys from both form data and files
                form_keys = list(request.form.keys())
                file_keys = list(request.files.keys())

            print( f"{photo_present, form_keys, file_keys}")
            return f"{photo_present, form_keys, file_keys}"
        
            if form.photo:
                return f"form photo field-> {[k for k in request.form.keys()]}"
                # return f"{form.photo.data}"
            else:
                # Debugging output for validation errors and all request data
                print("Form data: ", request.form)
                print("File data: ", request.files)
                for key, value in request.form.items():
                    print(f" {key}")
                    # print(f"File field: {key}, Value: {value}")
                return f"Form is not valid: {form.errors}"
                return f"form.photo.data is None: {request.files}"

            user.photo = save_image.save_photo(form.photo.data) or current_user.photo or '1.png'
